...
namespace Orange
{
	...
	TextBox::TextBox(const std::string text, const Vec2 position /*= Vec2(50, 50)*/, const Vec2 size /*= Vec2(200, 200)*/, const Vec2 padding /*= Vec2(10, 10)*/)
	...
	const UIComponentType TextBox::GetType() const
	{
		return UIComponentType::TEXTBOX;
	}

	const uint64_t TextBox::CalculateHash()
	{
		uint64_t hash = 0;
	...
	const uint64_t TextBox::CalculateHashFromText(const std::string text)
	{
		uint64_t hash = 0;
		...
		for (uint32_t i = 0; i < textBox->m_text.size(); i++)
		{
			...
			if (m_charToSRVMap[c] != nullptr)
			{
				...
				UIVertex newVertices[6] =
				{
					...
					{ QUAD_COORDINATES[0], size, newPos },
					{ QUAD_COORDINATES[1], size, newPos },
					{ QUAD_COORDINATES[2], size, newPos },
					{ QUAD_COORDINATES[3], size, newPos },
					{ QUAD_COORDINATES[4], size, newPos },
					{ QUAD_COORDINATES[5], size, newPos }
				};

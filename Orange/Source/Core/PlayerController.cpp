...
namespace Orange
{
	...
	void PlayerController::Update(const float& dt, Orange::Player* player)
	{
		// DEBUG
		...
		case CameraType::FirstPerson:
		{
			...
			std::function<float(float)> stopFunction = [=](const float val) 
			{
				...
				Orange::Math::Clamp(mappedVal, 0.0f, 1.0f);
				return mappedVal;
			};
			...
			if (Input::IsKeyDown(KeyCode::O) || Input::IsKeyDown(KeyCode::W)) // FORWARD
			{
				...
				if (currentVelocity.z < 0)
				{
					...
					Orange::Math::Clamp(stoppingLerpValues.z, 0.0f, 1.0f);
				...
				else
				{
					...
					Orange::Math::Clamp(startingLerpValues.z, 0.0f, 1.0f);
			...
			if (Input::IsKeyDown(KeyCode::K) || Input::IsKeyDown(KeyCode::A)) // LEFT
			{
				if (currentVelocity.x > 0)
				{
					...
					Orange::Math::Clamp(stoppingLerpValues.x, 0.0f, 1.0f);
				...
				else
				{
					...
					Orange::Math::Clamp(startingLerpValues.x, 0.0f, 1.0f);
			...
			if (Input::IsMouseDown(MouseCode::LBUTTON))
			{
				...
				deltaRotation.x = Input::GetMouseDelta().y * player->m_FPSCamera->GetRotationSpeed();
				// Rotation around the Y axis (look left/right)
				deltaRotation.y = Input::GetMouseDelta().x * player->m_FPSCamera->GetRotationSpeed();
			}

			// X Rotation
			player->m_rotation.y += deltaRotation.y;
			Orange::Math::Wrap(player->m_rotation.y, 0.0f, 360.0f);

			// Y Rotation
			player->m_rotation.x += deltaRotation.x;
			Orange::Math::Clamp(player->m_rotation.x, -89.9f, 89.9f);
			...
			XMMATRIX rotXMatrix = XMMatrixRotationX(Orange::Math::DegreesToRadians(player->m_rotation.x));
			XMMATRIX rotYMatrix = XMMatrixRotationY(Orange::Math::DegreesToRadians(player->m_rotation.y));
			...
			if (abs(currentVelocity.y) > TERMINAL_VELOCITY) currentVelocity.y = Orange::Math::Sign(currentVelocity.y) * TERMINAL_VELOCITY;
		...
		case CameraType::Debug:
		{
			...
			{
				// Rotation around the X axis (look up/down)
				deltaRotation.x = Input::GetMouseDelta().y * player->m_debugCamera->GetRotationSpeed();
				// Rotation around the Y axis (look left/right)
				deltaRotation.y = Input::GetMouseDelta().x * player->m_debugCamera->GetRotationSpeed();
			}

			// X Rotation
			rotation.y += deltaRotation.y;
			Orange::Math::Wrap(rotation.y, 0.0f, 360.0f);

			// Y Rotation
			rotation.x += deltaRotation.x;
			Orange::Math::Clamp(rotation.x, -89.9f, 89.9f);

			XMMATRIX localTransform = XMMatrixIdentity();
			XMMATRIX rotXMatrix = XMMatrixRotationX(Orange::Math::DegreesToRadians(rotation.x));
			XMMATRIX rotYMatrix = XMMatrixRotationY(Orange::Math::DegreesToRadians(rotation.y));
			XMMATRIX translationMatrixXZ = XMMatrixTranslation(deltaTranslation.x, 0.0f, deltaTranslation.z);
			localTransform = localTransform * rotYMatrix;
			localTransform = rotXMatrix * localTransform;
		...
		if (isCollidingWithWall)
		{
			...
			for (uint32_t axis = 0; axis <= 2; axis += 2)
			{
				...
				if (Physics::DetectCollision(currentPos))
				{
					...
					XMFLOAT3_BRACKET_OP_32(velocityToAdd, axis) = Orange::Math::Sign(XMFLOAT3_BRACKET_OP_32(velocityToAdd, axis)) * (abs(XMFLOAT3_BRACKET_OP_32(velocityToAdd, axis)) - overlap);
					OG_ASSERT(overlap > 0);
			...
			OG_ASSERT((abs(velocityToAdd.x) >= 0 && abs(velocityToAdd.x) < 1));
			OG_ASSERT((abs(velocityToAdd.z) >= 0 && abs(velocityToAdd.z) < 1));
		}
	...
	XMFLOAT3 PlayerController::CheckForVerticalCollision(AABB& newPos, const AABB& prevPos)
	{
		...
		if (isCollidingWithFloor)
		{
			...
			OG_ASSERT(overlap > 0);
			velocityToAddOnY = Orange::Math::Sign(velocityToAddOnY) * (abs(velocityToAddOnY) - overlap);

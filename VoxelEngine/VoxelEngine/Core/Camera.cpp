...
void Camera::Update(float dt)
{
	...
	for (uint32_t i = 0; i < 2; i++)
	{
		...
		if (abs(deltaAxisRotation) > rotationWrapThreshold)
		{
			...
			int32_t sign = VX_MATH::Sign(deltaAxisRotation);
			if (sign == -1)
			{
	...
	for (uint32_t i = 0; i < 2; i++)
	{
		...
		VX_MATH::Clamp(ratio, 0.0f, 1.0f);
		XMFLOAT3_BRACKET_OP_32(m_currentRotation, i) = VX_MATH::Lerp(XMFLOAT3_BRACKET_OP_32(m_currentRotation, i), XMFLOAT3_BRACKET_OP_32(finalRotationValues, i), ratio);
	}
	VX_MATH::Wrap(m_currentRotation.y, 0.0f, 360.0f);
...
void Camera::ConstructMatrix(const XMFLOAT3& pos, const XMFLOAT3& rot)
{
	...
	XMMATRIX rotationMatrix = XMMatrixRotationRollPitchYaw(VX_MATH::DegreesToRadians(rot.x), VX_MATH::DegreesToRadians(rot.y), VX_MATH::DegreesToRadians(rot.z));
